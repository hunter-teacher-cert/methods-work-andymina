...
public class Solution {
  ...
  */
  public static int[] selectionSort(int[] numbers) {
    // loop through backward to make sure we don't accidentally swap an element
    // after it was swapped into place
    for (int idxToSwap = numbers.length - 1; idxToSwap > 0; idxToSwap--) {
      // find the index of the max using the helper function
      int maxIdx = findIndexOfMax(idxToSwap);
      
      // swap it into place
      swap(numbers, idxToSwap, maxIdx);
    }
  }
